...
[execute][template]header.tpl[/template][/execute]
        <img src="/images/logo.png" alt="[gettext]Logo[/gettext]" style="float: left; padding: 20px;" />
        ...
        <p>[gettext]Due to this innovation, we now have to worry about two different types of languages: Client-side and Server-side. Some of the (X)HTML, CSS, and Javascript, which the client-side browser will interpret, can stay static on the page, while the changing parts need to be generated by a server-side language like PHP or Python. This results in several languages on the same document, which can get <a href="[url controller="root" action="template" templatefile="docs" parameter1="comparison" /]">very messy</a>, and forces all the developers to work on one file. By default, CSS and Javascript can be stored in an external file, but what can we do about the server-side language?[/gettext]</p>

        <p>[gettext]Introducing the <strong>SUIT Framework</strong> (Scripting Using Integrated Templates), an open-source web <a href="http://en.wikipedia.org/wiki/Template_engine_(web)">templating framework</a> with some <a href="[url controller="root" action="template" templatefile="docs" parameter1="features" /]">innovative features</a>. It provides a lightweight and powerful solution to this problem by allowing developers to define their own syntax for transforming templates by using <a href="[url controller="root" action="template" templatefile="docs" parameter1="rules" /]">rules</a>. The rules should generally allow the transfer of information from the server-side language to the template. Then, <a href="[url controller="root" action="template" templatefile="docs" parameter1="execute" /]">execute</a> the template according to the provided rules, and problem solved.[/gettext]</p>

        <p>[gettext]SUIT currently has both PHP and Python implementations, and provides several sets of rules for both in our <a href="[url controller="root" action="template" templatefile="download" /]#rulebox">Rulebox</a>. Although the different versions have some differences in setting up the rules and executing, the templates do not directly evaluate in those languages, making our solution safer than many others. In addition, templates used with SUIT for PHP can be used with SUIT for Python.[/gettext]</p>

        <p>[gettext]So, read the <a href="[url controller="root" action="template" templatefile="docs" /]">Docs</a>, get help in the <a href="https://sourceforge.net/projects/suitframework/forums" target="_blank">Forums</a>, and enjoy clean coding![/gettext]</p>
